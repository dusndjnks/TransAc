...
const Header = () => {
  ...
  return (
   <header className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? "bg-[#fff5eb] shadow-sm" : "bg-transparent"}`}>
    ...
    <div className="hidden md:block py-2 bg-transparent">
      <div className="max-w-7xl mx-auto px-4 flex justify-between items-center">
        <div className="flex space-x-6">
          <a href="tel:+9530123654896" className="flex items-center text-sm text-white">
            <FaPhone className="mr-2" />
            +953 012 3654 896
          </a>
          <a href="mailto:transac@gmail.com" className="flex items-center text-sm text-white">
        ...
        <div className="flex space-x-4">
          {socialLinks.map((social, index) => (
            <a key={index} href={social.url} className="text-white hover:opacity-80">
              {social.icon}
            </a>
          ))}
        </div>
  ...
  <div className="max-w-7xl mx-auto px-4">
    <div className="flex justify-between items-center h-16">
      ...
      <NavLink to="/" className="flex items-center">
        ...
        <span className={`text-xl font-medium ${isScrolled ? "text-gray-900" : "text-white"}`}>
          Trans India Cooling
        </span>
      </NavLink>
        ...
        {navLinks.map((link) => (
          <NavLink
            ...
            className={({ isActive }) => `
              ...
              ${isActive ? 
                (isScrolled ? "text-gray-900 border-b-2 border-[#f8821e]" : "text-white border-b-2 border-white") : 
                (isScrolled ? "text-gray-600 hover:text-[#f8821e]" : "text-white hover:text-gray-200")}
            `}
          >
            {link.label}
          </NavLink>
